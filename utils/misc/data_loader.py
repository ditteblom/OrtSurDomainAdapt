...
class XrayData(torch.utils.data.Dataset):
    def __init__(self, repair_type, split, source_or_target, data_path = "data/", transform = None, train_size = 0.8, test_size = 0.2, seed = 8, annotations = False):
        ...
        if self.type == "source":
            ...
            image_test_df = pd.DataFrame([image_test,score_test])
            image_test_df.to_csv(data_path + self.type + '_' + repair_type +'_testdata.csv')
            image_val_df = pd.DataFrame([image_val,score_val])
            image_val_df.to_csv(data_path + self.type + '_' + repair_type +'_valdata.csv')
            image_train_df = pd.DataFrame([image_train,score_train])
            image_train_df.to_csv(data_path + self.type + '_' + repair_type +'_traindata.csv')
            ...
            img_files = glob.glob(self.data_path + repair_type + '/*.tiff')
            img_files = [ x for x in img_files if 'ap' in x ] # only frontal image

            # divide dataset into train, val and test
            image_trainval, image_test = train_test_split(img_files, test_size=test_size, train_size=train_size, random_state=seed)
            image_train, image_val = train_test_split(image_trainval, test_size=0.2, train_size=0.8, random_state=seed)

            # save train and test as csv files
            image_test_df = pd.DataFrame(image_test)
            image_test_df.to_csv(data_path + self.type + '_' + repair_type +'_testdata.csv')
            image_val_df = pd.DataFrame(image_val)
            image_val_df.to_csv(data_path + self.type + '_' + repair_type +'_valdata.csv')
            ...
            # devide images into train, validation and test set
            if split == "train":
                self.images = image_train
            elif split == "val":
                self.images = image_val
            elif split == "test":
...
def get_loader(repair_type, split, source_or_target, data_path = "data/", batch_size=16, transform = None, num_workers=0, shuffle = True, seed = 8):
